...
The local MCP server exposes one investigation tool that delegates to the Investigation Command. The React UI renders Machine Report JSON and can import fresh reports. The Local LLM Narrative summarizes Machine Report facts only and falls back to a Deterministic Narrative. Datadog and PagerDuty are modeled as optional Operational Enrichment; when live Coral enrichment is unavailable, fallback rows are labeled Demo Enrichment Data.
